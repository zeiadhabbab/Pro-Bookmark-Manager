document.addEventListener("DOMContentLoaded", () => {
  ...
  const updateQuickAccessTabs = () => {
    ...
    const recentlyAddedTab = document.querySelector(
    ...
    if (recentlyAddedTab) {
      recentlyAddedTab.style.display = settings.showRecentlyAdded
        ? "flex"
        : "none";
    }
  };
  ...
  const setupEventListeners = () => {
    ...
    // Edit bookmark form
    document
      .getElementById("updateBookmark")
      .addEventListener("click", handleUpdateBookmark);
    document
      .getElementById("deleteBookmark")
      .addEventListener("click", handleDeleteBookmark);
    document
  ...
  const loadInitialBookmarks = async () => {
    ...
    try {
      ...
      const rootFolders = bookmarkTree[0].children.filter(
        (node) => node.children && node.title
      );
  ...
  const populateFolderDropdowns = () => {
    ...
    dropdowns.forEach((dropdown) => {
      dropdown.innerHTML = `<option value="" data-i18n="modal.fields.selectFolder">${translate(
        "modal.fields.selectFolder"
      )}</option>`;
    ...
    for (const node of nodes) {
      if (node.children) {
        ...
        folders = folders.concat(
          getFolderList(node.children, prefix + node.title + " / ")
        );
      }
    }
    return folders;
  ...
  const handleSearch = (event) => {
    ...
    chrome.search.query({
      text: query,
      disposition: 'NEW_TAB' // Optional: opens the results in a new tab
    })
  ...
  const handleFilterChange = () => {
    ...
    switch (filterValue) {
      case "today":
        filteredBookmarks = currentBookmarks.filter(
          (bookmark) =>
      ...
      case "week":
        filteredBookmarks = currentBookmarks.filter(
          (bookmark) =>
            bookmark.dateAdded &&
            now - bookmark.dateAdded < 7 * 24 * 60 * 60 * 1000
        );
        break;
      case "month":
        filteredBookmarks = currentBookmarks.filter(
          (bookmark) =>
            bookmark.dateAdded &&
            now - bookmark.dateAdded < 30 * 24 * 60 * 60 * 1000
        );
        break;
    }
  ...
  const handleSaveBookmark = () => {
    ...
    chrome.bookmarks.create(bookmarkData, (result) => {
      if (chrome.runtime.lastError) {
        showToast(
          translate("toast.bookmarkCreateError", {
            message: escapeHtml(chrome.runtime.lastError.message),
          }),
          "error"
        );
      } else {
  ...
  const handleUpdateBookmark = () => {
    ...
    chrome.bookmarks.update(bookmarkId, { title, url }, (result) => {
      if (chrome.runtime.lastError) {
        showToast(
          translate("toast.bookmarkUpdateError", {
            message: escapeHtml(chrome.runtime.lastError.message),
          }),
          "error"
        );
  ...
  const handleDeleteBookmark = () => {
    ...
    if (confirm(translate("confirm.deleteBookmark"))) {
      chrome.bookmarks.remove(bookmarkId, () => {
        if (chrome.runtime.lastError) {
          showToast(
            translate("toast.bookmarkDeleteError", {
              message: escapeHtml(chrome.runtime.lastError.message),
            }),
            "error"
          );
  ...
  const renderFolderTabs = (folders, container, isPrimary) => {
    let tabsHtml = isPrimary
      ? `<button class="folder-tab" data-id="all">
  ...
  const renderBookmarks = (bookmarks, container, displayType = "grid") => {
    if (!bookmarks || bookmarks.length === 0) {
      container.innerHTML = `
                <div style="grid-column: 1 / -1; text-align: center; padding: 40px; opacity: 0.7;">
                    ...
                    <p style="font-size: 16px; margin: 0;">${translate(
                      "empty.noBookmarks"
                    )}</p>
                </div>
            `;
    ...
    container.innerHTML = bookmarks
      .map((bookmark) => {
        ...
        const displayUrl = fallbackHostname
          ? translate("bookmark.unknownHost")
          : hostname;
        const safeDisplayUrl = escapeHtml(displayUrl);

        if (displayType === "grid") {
          return `
                <div class="bookmark" data-bookmark-id="${
                  bookmark.id
                }" draggable="true">
                    ...
                    <div class="bookmark-content">
                        <h3>${
                          ...
                          translate("bookmark.untitled")
                        }</h3>
                        <div class="url">${safeDisplayUrl}</div>
                    </div>
                    <div class="bookmark-actions">
                        <button class="bookmark-action open" title="${translate(
                          "bookmark.action.open"
                        )}">
                            <i class="fas fa-external-link"></i>
                        </button>
                        <button class="bookmark-action edit" title="${translate(
                          "bookmark.action.edit"
                        )}">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="bookmark-action delete" title="${translate(
    ...
    container.querySelectorAll(".bookmark").forEach((bookmarkElement) => {
      ...
      // Click to open bookmark in current tab
        ...
        deleteBtn.addEventListener("click", (e) => {
          ...
          if (confirm(translate("confirm.deleteBookmark"))) {
            chrome.bookmarks.remove(bookmarkId, () => {
              if (chrome.runtime.lastError) {
                showToast(
                  translate("toast.bookmarkDeleteGenericError"),
                  "error"
                );
              } else {
                showToast(translate("toast.bookmarkDeleted"), "success");
  ...
  const getCurrentBookmarks = () => {
    switch (currentView) {
      case "most-visited":
        return Array.from(mostVisitedGrid.querySelectorAll(".bookmark")).map(
          (el) => ({
      ...
      case "recently-added":
        return Array.from(recentlyAddedGrid.querySelectorAll(".bookmark")).map(
          (el) => ({
            id: el.dataset.bookmarkId,
            title: el.querySelector("h3").textContent,
            url: el.querySelector(".url").textContent,
  ...
  const restorePreviousView = () => {
    ...
    if (activeSubTab) {
      ...
      const activePrimaryTab =
        primaryFolderTabs.querySelector(".folder-tab.active");
      if (activePrimaryTab) {
